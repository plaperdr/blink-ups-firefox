...
function importPasswordsFromJSON(){
	var masterPassword = {value: ""};
	prompts.promptPassword(null,"Passwords decryption","Enter the correct Master Password to decrypt your current passwords", masterPassword,null,{value: false});
	while(masterPassword.value != "" && CryptoSHA.SHA3(masterPassword.value) != importJSONData.masterPassword){
		prompts.promptPassword(null,"Passwords decryption","Enter the correct Master Password to decrypt your current passwords\nIncorrect password - Try again", masterPassword,null,{value: false});
	}
	
	if(masterPassword.value != ""){
		for(i=0 ; i<importJSONData.passwords.length ; i++){
			var hostname = CryptoAES.AES.decrypt(importJSONData.passwords[i].hostname,masterPassword.value).toString(CryptoAES.enc.Utf8);
			var formSubmitURL = CryptoAES.AES.decrypt(importJSONData.passwords[i].formSubmitURL,masterPassword.value).toString(CryptoAES.enc.Utf8);
			var username = CryptoAES.AES.decrypt(importJSONData.passwords[i].username,masterPassword.value).toString(CryptoAES.enc.Utf8);
			var password = CryptoAES.AES.decrypt(importJSONData.passwords[i].password,masterPassword.value).toString(CryptoAES.enc.Utf8);
...
function exportPasswordsToJSON(){
	...
	var masterPassword = {value: ""};
	prompts.promptPassword(null,"Passwords encryption","Enter a Master Password to encrypt your passwords\n(Must be at least 5 characters long)", masterPassword,null,{value: false});
	while (masterPassword.value.length < 5){
		prompts.promptPassword(null,"Passwords encryption","Enter a Master Password to encrypt your passwords\n(Must be at least 5 characters long)", masterPassword,null,{value: false});
	}
	
	exportJSONData.masterPassword = ''+CryptoSHA.SHA3(masterPassword.value);
	for (var i = 0; i < logins.length; i++) {
		exportJSONData.passwords.push(
				{hostname : ''+ CryptoAES.AES.encrypt(logins[i].hostname, masterPassword.value), 
				 username : ''+ CryptoAES.AES.encrypt(logins[i].username, masterPassword.value), 
				 password : ''+ CryptoAES.AES.encrypt(logins[i].password, masterPassword.value),
				 formSubmitURL : ''+ CryptoAES.AES.encrypt(logins[i].formSubmitURL, masterPassword.value)}
		);
